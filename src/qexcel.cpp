...
bool QExcel::add(const QString &filePath)
{
    if (workBooks)
    {
        workBooks->dynamicCall("Add");
        //保存至filepath，注意一定要用QDir::toNativeSeparators将路径中的"/"转换为"\"，不然一定保存不了。
        workBooks->dynamicCall("SaveAs(const QString&)",QDir::toNativeSeparators(filePath));
        workBook = excel->querySubObject("ActiveWorkBook");
        if (!workBook)
        {
            return false;
...
bool QExcel::open(const QString &filePath)
{
    if (workBooks)
    {
